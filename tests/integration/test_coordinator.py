...
class TestSolarGuardianDataUpdateCoordinator(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_update_data_api_error(self):
        ...
        self.mock_api.authenticate.return_value = True
        self.mock_api.get_power_stations.side_effect = SolarGuardianAPIError("API Error")
    ...
    async def test_update_data_device_processing(self):
        ...
        self.assertEqual(len(device_data["variableGroupList"]), len(MOCK_VARIABLE_GROUPS))
    ...
    async def test_update_data_partial_failure(self):
        ...
        # Latest data fails
        self.mock_api.get_latest_data.side_effect = SolarGuardianAPIError("Latest data failed")
...
class TestCoordinatorMockMode(unittest.TestCase):
    ...
    def test_mock_data_stations(self):
        ...
        self.assertEqual(stations[0]["powerStationName"], MOCK_POWER_STATION["powerStationName"])
    ...
    def test_mock_data_device_parameters(self):
        ...
        variable_groups = mock_data["device_data"][device_id]["data"]["variableGroupList"]
        self.assertEqual(len(variable_groups), len(MOCK_VARIABLE_GROUPS))
        
        # Verify structure matches expected format
        for i, group in enumerate(variable_groups):
            self.assertEqual(group["variableGroupNameE"], MOCK_VARIABLE_GROUPS[i]["variableGroupNameE"])
            self.assertEqual(len(group["variableList"]), len(MOCK_VARIABLE_GROUPS[i]["variableList"]))
