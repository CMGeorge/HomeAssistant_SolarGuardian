...
class TestMockDataStructure(unittest.TestCase):
    ...
    def test_mock_data_realistic_values(self):
        ...
        if "capacity" in MOCK_POWER_STATION:
            ...
            self.assertLess(
                capacity, 1000000
            )  # Less than 1MW seems reasonable for test
        ...
        power_params = [
            v for v in all_variables if "power" in v["variableNameE"].lower()
        ]
        voltage_params = [
            v for v in all_variables if "voltage" in v["variableNameE"].lower()
        ]
        current_params = [
            v for v in all_variables if "current" in v["variableNameE"].lower()
        ]
...
class TestErrorHandlingData(unittest.TestCase):
    ...
    def test_empty_variable_lists(self):
        ...
        empty_group = {"variableGroupNameE": "Empty Group", "variableList": []}
