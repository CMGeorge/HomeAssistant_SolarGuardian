...
def validate_manifest(manifest_path: Path) -> list[str]:
    ...
    if manifest.get("domain") != manifest_path.parent.name:
        errors.append(f"❌ Domain '{manifest.get('domain')}' doesn't match directory name '{manifest_path.parent.name}'")
...
def validate_translations(integration_path: Path) -> list[str]:
    ...
    if not en_json.exists():
        errors.append("❌ translations/en.json not found (required if translations/ exists)")
